...
export interface HealthDisplayProps {
 ...
 disabilities?:string;
 medication?:string;
hasHealthProps:boolean;
}

export default function HealthDisplay({
hasAllergies,hasDisabilities,hasMedication,hasPreExisting,preExisitng,allergies,disabilities,medication,hasHealthProps
}: HealthDisplayProps) {
  ...
  >
    ...
    {hasHealthProps?null:<Grid item xs={12} sx={{display:'flex',justifyContent:'center'}}><Box sx={{width:'100%',height:'100px'}}><Typography fontSize={24}>Health</Typography><Box sx={{height:'100px',width:'100%',backgroundColor:"#DAE1E9",borderRadius:'5px',overflowY:'auto',overflowWrap: 'break-word'}}><Typography fontSize={15} sx={{ml:1}}>No Data Available, as you did not set any requirements for this study</Typography></Box></Box></Grid>}
    </Grid>
